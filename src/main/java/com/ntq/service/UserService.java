...
//                u.setRegdate(d);
    ...
    public List<User>userBanded (String kw)
    {
        try(Session session = FACTORY.openSession())
        {
            CriteriaBuilder b = session.getCriteriaBuilder();
            ...
            if(kw !=null && !kw.isEmpty())
            {
                ...
                Predicate p1 = b.like(root.get("fullname").as(String.class), p);
                Predicate p2 = b.like(root.get("email").as(String.class), p);
                q = q.where((b.or(p1,p2)));
            }
            q = q.where(b.equal(root.get("band").as(Integer.class), 1));
