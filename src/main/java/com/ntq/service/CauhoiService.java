...
//            Join<Lichsu,User> u = l.join("users", JoinType.INNER);
    ...
    public boolean deleteCauhoi(Cauhoi c)
    {
        try(Session session = FACTORY.openSession())
        {
            try{
               session.getTransaction().begin();
               CriteriaBuilder b = FACTORY.getCriteriaBuilder();
                CriteriaDelete<Lichsu> q = b.createCriteriaDelete(Lichsu.class);
                Root<Lichsu> cc = q.from(Lichsu.class);
                q.where(b.equal(cc.get("cauhoi").as(Cauhoi.class), c));
                session.createQuery(q).executeUpdate();
                session.delete(c);
                session.getTransaction().commit();
                return true;
//                
//                int a= session.createQuery(q).executeUpdate();
