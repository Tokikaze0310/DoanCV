        ...
        try(Session session = FACTORY.openSession())
        {
            ...
            if (orderby != 0) {
                switch (orderby) {
                    ...
                    case 4:
                        if (!typesort) {
                            q.orderBy(b.asc(root.get("status").as(Integer.class)));
                        } else {
                            q.orderBy(b.desc(root.get("status").as(Integer.class)));
                        }
                        break;
                    case 5:
                        if (!typesort) {
                            q.orderBy(b.asc(root.get("tongdiem").as(Integer.class)));
                        } else {
                            q.orderBy(b.desc(root.get("tongdiem").as(Integer.class)));
                        }
                        break;
                    case 6:
                        if (!typesort) {
                            q.orderBy(b.asc(root.get("time").as(Date.class)));
                        } else {
                            q.orderBy(b.desc(root.get("time").as(Date.class)));
                        }
                        break;
                }
